...
def chatbot(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Keep the prompt concise for a fast medical-education response.
            prompt = (
                "Medical assistant: Answer this medical education question "
                f"in 2-3 sentences: {user_input}"
            )

            # Generate response with timeout/error handling
            try:
                ...
                if not response.text:
                    return JsonResponse({
                        'reply': (
                            'I apologize, but I cannot provide a response to '
                            'that question. Please try rephrasing your medical '
                            'question.'
                        )
                    })
                ...
                if "quota" in error_msg.lower():
                    ...
                    return JsonResponse({
                        'reply': (
                            'Response took too long. Please try a shorter '
                            'question.'
                        )
                    })
            ...
            response_time = time.time() - start_time
            print(
                f"Error after {response_time:.2f} seconds: {str(e)}"
            )
